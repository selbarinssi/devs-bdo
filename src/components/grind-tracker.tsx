      ...
      <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
        <aside className="glass flex flex-col gap-2 p-3">
          <div className="mb-1 flex items-center justify-between">
            ...
            <button type="button" onClick={() => { setAddingSpot((v) => !v); setEditingSpot(false); setSpotName(""); setSpotMonsters(""); setSpotTerritory(""); setSpotIconUrl(null); setSpotMode("pve"); }} className="btn-ghost h-7 px-2 text-[0.65rem]">
          ...
          {addingSpot && (
            <div className="mb-1 flex flex-col gap-1.5 rounded-lg bg-white/5 p-2">
              ...
              <div className="grid grid-cols-2 gap-1">
                <button type="button" onClick={() => setSpotMode("pve")} className={cn("h-8 rounded-md text-[0.65rem] font-semibold ring-1 transition", spotMode === "pve" ? "bg-cyan-400/15 text-cyan-200 ring-cyan-400/40" : "bg-white/5 text-muted-foreground ring-white/10")}>Monsters</button>
                <button type="button" onClick={() => setSpotMode("lifeskill")} className={cn("h-8 rounded-md text-[0.65rem] font-semibold ring-1 transition", spotMode === "lifeskill" ? "bg-emerald-400/15 text-emerald-200 ring-emerald-400/40" : "bg-white/5 text-muted-foreground ring-white/10")}>Lifeskill</button>
              </div>
              <select className="field-select h-8 w-full text-xs" value={spotTerritory} onChange={(e) => setSpotTerritory(e.target.value)}>
                <option value="">Region…</option>
                {TERRITORIES.map((r) => (<option key={r} value={r}>{r}</option>))}
              </select>
              <select className="field-select h-8 w-full text-xs" value={spotMonsters} onChange={(e) => setSpotMonsters(e.target.value)}>
                ...
                {(spotMode === "lifeskill" ? LIFESKILL_TYPES : MONSTER_TYPES).map((r) => (<option key={r} value={r}>{r}</option>))}
              </select>
              <button type="button" onClick={async () => setSpotIconUrl(await pickIconFile())} className="btn-ghost flex h-8 items-center justify-center gap-1.5 text-[0.65rem]">
                <ImagePlus className="size-3.5" />{spotIconUrl ? "Change Icon" : "Add Icon"}
              </button>
          ...
          <div className="flex max-h-[55vh] flex-col gap-3 overflow-auto">
            {spotsByRegion.map(([terr, list]) => (
              <div key={terr}>
                ...
                <ul className="flex flex-col gap-1">
                  {list.map((s) => (
                    <li key={s.id}>
                      <button type="button" onClick={() => setSelectedId(s.id)} className={cn("flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left text-sm transition", selectedId === s.id ? "bg-cyan-400/15 text-cyan-200 ring-1 ring-cyan-400/30" : "hover:bg-white/5 text-foreground")}>
                        <span className="flex min-w-0 items-center gap-2">
                          {s.icon_url ? <img src={s.icon_url} alt="" className="size-8 shrink-0 rounded object-contain" /> : <span className="flex size-8 shrink-0 items-center justify-center rounded bg-white/5 text-[0.65rem] font-bold text-cyan-300">{s.name[0]?.toUpperCase()}</span>}
                          <span className="truncate font-medium">{s.name}</span>
                        </span>
                        <button type="button" className="ml-1 text-muted-foreground hover:text-rose-400" onClick={async (e) => { e.stopPropagation(); await deleteSpot(s.id); setSpots((p) => p.filter((x) => x.id !== s.id)); if (selectedId === s.id) setSelectedId(null); }}>
            ...
            {filteredSpots.length === 0 && <p className="px-2 py-6 text-center text-xs text-muted-foreground">No Spots Match</p>}
          </div>
        </aside>
            ...
            <>
              <div className="glass flex flex-wrap items-center justify-between gap-2 p-3">
                <div className="flex min-w-0 items-center gap-2.5">
                  {selectedSpot?.icon_url ? <img src={selectedSpot.icon_url} alt="" className="size-10 rounded object-contain" /> : <span className="flex size-10 items-center justify-center rounded bg-white/5 text-sm font-bold text-cyan-300">{selectedSpot?.name[0]?.toUpperCase()}</span>}
                  <div className="min-w-0">
                    <h3 className="truncate text-base font-semibold sm:text-lg">{selectedSpot?.name}</h3>
                    <p className="truncate text-xs text-muted-foreground sm:text-sm">{selectedSpot?.monsters}{selectedSpot?.territory ? ` · ${selectedSpot.territory}` : ""}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {avgSph > 0 && <span className="metric-pill bg-emerald-400/10 text-sm text-emerald-300">Avg {formatSilver(avgSph)}/h</span>}
                  <button type="button" onClick={beginEditSpot} className="btn-ghost h-8 px-2.5 text-xs"><Pencil className="size-3.5" /> Edit Spot</button>
                </div>
              </div>
              ...
              {editingSpot && (
                <div className="glass grid gap-2 p-3 sm:grid-cols-2">
                  ...
                  <button type="button" onClick={async () => setSpotIconUrl(await pickIconFile())} className="btn-ghost flex h-9 items-center justify-center gap-1.5 text-xs">
                    <ImagePlus className="size-4" />{spotIconUrl ? "Change Icon" : "Add Icon"}
                  </button>
              ...
              <ChronoPanel hh={hh} mm={mm} ss={ss} timerOn={timerOn} total={sessionTotals.total} sph={sessionTotals.sph} character={character} dropRate={dropRate} minutes={minutes} onToggle={toggleTimer} onReset={resetTimer} onCharacter={setCharacter} onDropRate={setDropRate} onMinutes={setMinutes} />

              <div className="glass p-3 sm:p-4">
                <div className="mb-2 flex items-center justify-between">
                  <p className="text-[0.7rem] font-bold uppercase tracking-wider text-muted-foreground">Loot</p>
                  <button type="button" onClick={() => setAddingLoot((v) => !v)} className="btn-ghost h-8 px-2.5 text-xs"><Plus className="size-3.5" /> Add</button>
                </div>
                {loots.length > 0 && (
                  <div className="mb-1.5 grid grid-cols-[minmax(0,1fr)_4.25rem_5.25rem_4rem_5.25rem_4.5rem] items-center gap-1.5 px-2 text-[0.6rem] font-bold uppercase tracking-wider text-muted-foreground sm:grid-cols-[minmax(0,1fr)_5rem_6rem_4.5rem_6rem_5rem] sm:gap-2">
                    <span>Item</span><span className="text-center">Qty</span><span className="text-right">Total</span><span className="text-right">%</span><span className="text-right">/h</span><span />
                  </div>
                )}
                {addingLoot && (
                  <div className="mb-2 grid grid-cols-2 gap-1.5 rounded-lg bg-white/5 p-2 sm:grid-cols-3 lg:grid-cols-6">
                    ...
                    <select className="field-select h-9 text-sm" value={lootKind} onChange={(e) => setLootKind(e.target.value as "market" | "npc")}>
                      <option value="market">Market</option><option value="npc">NPC</option>
                    </select>
                    <select className="field-select h-9 text-sm" value={lootRarity} onChange={(e) => setLootRarity(e.target.value as LootRarity)}>
                    ...
                    <button type="button" onClick={async () => setLootIconUrl(await pickIconFile())} className="btn-ghost flex h-9 items-center justify-center gap-1 text-xs"><ImagePlus className="size-4" /> Icon</button>
                ...
                <ul className="flex flex-col gap-1.5">
                      ...
                      <li key={l.id} className="rounded-xl bg-white/[0.03] px-2.5 py-2">
                        {editing ? (
                          <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-6">
                            ...
                            <select className="field-select h-9 text-sm" value={editLootRarity} onChange={(e) => setEditLootRarity(e.target.value as LootRarity)}>
                              {RARITY_ORDER.map((r) => (<option key={r} value={r}>{RARITY_LABEL[r]}</option>))}
                            </select>
                            <Input type="number" value={editLootPrice} onChange={(e) => setEditLootPrice(e.target.value)} className="h-9 text-sm" />
                            <button type="button" onClick={async () => setEditLootIconUrl(await pickIconFile())} className="btn-ghost flex h-9 items-center justify-center gap-1 text-xs"><ImagePlus className="size-4" /> Icon</button>
                          ...
                          <div className="grid grid-cols-[minmax(0,1fr)_4.25rem_5.25rem_4rem_5.25rem_4.5rem] items-center gap-1.5 sm:grid-cols-[minmax(0,1fr)_5rem_6rem_4.5rem_6rem_5rem] sm:gap-2">
                            <div className="flex min-w-0 items-center gap-2.5">
                              {l.icon_url ? <img src={l.icon_url} alt="" className="size-9 shrink-0 rounded object-contain sm:size-10" /> : <span className="flex size-9 shrink-0 items-center justify-center rounded bg-white/5 text-xs font-bold text-cyan-300 sm:size-10">{l.name[0]?.toUpperCase()}</span>}
                              <div className="min-w-0">
                                <p className="truncate text-sm font-semibold text-foreground sm:text-[0.95rem]">{l.name}</p>
                                <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs text-muted-foreground">
                                  <span>{formatSilver(Number(l.unit_price))}</span>
                            ...
                            <div className="flex items-center justify-end gap-0.5">
                              <button type="button" onClick={() => beginEditLoot(l)} className="flex size-8 items-center justify-center rounded text-muted-foreground hover:text-cyan-300"><Pencil className="size-3.5" strokeWidth={1.75} /></button>
                              <button type="button" onClick={async () => { await deleteLoot(l.id); if (selectedId) setLoots(sortLootsByRarity(await listLoots(selectedId))); }} className="flex size-8 items-center justify-center rounded text-muted-foreground hover:text-rose-400"><Trash2 className="size-3.5" strokeWidth={1.75} /></button>
                            </div>
                          </div>
                        )}
                      </li>
                  ...
                  {loots.length === 0 && <li className="px-2 py-4 text-center text-sm text-muted-foreground">No Loot Yet. Add Items Above.</li>}
                </ul>
                {loots.length > 0 && (
                  <button type="button" onClick={() => setImportOpen(true)} className="btn-ghost mt-3 flex h-11 w-full items-center justify-center gap-2 text-sm">
                    <ImagePlus className="size-4" /> Update Quantities
                  </button>
                )}
                <button type="button" onClick={onSaveSession} disabled={busy || sessionTotals.total <= 0} className="btn-primary mt-2 h-11 w-full text-sm">Save Session</button>
              </div>
              ...
              <div className="glass p-3 sm:p-4">
                ...
                {spotSessions.length === 0 ? (
                  ...
                  <ul className="flex flex-col gap-1.5">
                    {spotSessions.map((s) => {
                      const savedLabel = (() => { try { return new Date(s.created_at).toLocaleString(undefined, { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }); } catch { return s.created_at; } })();
                      return (
