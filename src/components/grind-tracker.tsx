            ...
            <>
                ...
                <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
                  ...
                  <div className="flex items-center gap-4 rounded-2xl border border-cyan-400/15 bg-black/25 px-4 py-2">
                    <div className="text-center">
                      ...
                      <p className={cn("font-mono text-2xl font-semibold tabular-nums tracking-wider", timerOn ? "neon-text" : "text-foreground")}>{hh}:{mm}:{ss}</p>
                      <div className="mt-1 flex justify-center gap-1">
                        ...
                        <button type="button" onClick={resetTimer} className="rounded-full px-2 py-0.5 text-[0.65rem] text-muted-foreground ring-1 ring-white/10">Reset</button>
                      </div>
                    </div>
                  ...
                  {loots.map((l) => {
                    ...
                    return (
                      <div key={l.id} className="flex items-center gap-2 rounded-xl border border-white/5 bg-black/20 px-2.5 py-1.5">
                        {l.icon_url ? <img src={l.icon_url} alt="" className="size-7 rounded object-contain" /> : <span className="size-7 rounded bg-white/5" />}
                        <span className="min-w-0 flex-1 truncate text-sm">{l.name}</span>
                        <span className="hidden text-[0.65rem] tabular-nums text-muted-foreground sm:inline">{formatSilver(Number(l.unit_price))} ⚙</span>
                        <Input type="number" min={0} placeholder="0" value={qty[l.id] ?? ""} onChange={(e) => setQty((prev) => ({ ...prev, [l.id]: e.target.value }))} className="h-7 w-16 text-right text-sm" />
                        <span className="w-20 text-right text-xs font-semibold tabular-nums text-violet-300">{q > 0 ? `${formatSilver(lineSph)}/h` : "—"}</span>
                      </div>
                    );
                  })}
                  {loots.length === 0 ? <p className="py-2 text-center text-xs text-muted-foreground">Add loot rows below</p> : null}
                </div>

                <div className="mt-3">
                  <button type="button" disabled={busy || loots.length === 0} onClick={() => void onSaveSession()} className="btn-primary">Save session</button>
                </div>
              </section>
              ...
              <section className="glass overflow-hidden">
                <div className="flex items-center justify-between border-b border-white/10 px-3 py-2">
                  ...
                  <button type="button" className="btn-ghost h-7 text-[0.65rem]" onClick={() => setAddingLoot((v) => !v)}><Plus className="size-3" /> Loot</button>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[480px] table-fixed text-left text-sm">
                    <colgroup><col className="w-[48%]" /><col className="w-[14%]" /><col className="w-[24%]" /><col className="w-[14%]" /></colgroup>
                    <thead>
                    ...
                    <tbody>
                      {loots.map((l) => (
                        <tr key={l.id} className="border-b border-white/5">
                          <td className="px-3 py-2"><div className="flex items-center gap-2">{l.icon_url ? <img src={l.icon_url} alt="" className="size-6 rounded object-contain" /> : <span className="size-6 rounded bg-white/5" />}<span className="truncate">{l.name}</span></div></td>
                          ...
                          <td className="px-2 py-2 text-right"><button type="button" className="text-muted-foreground hover:text-rose-400" onClick={() => { void (async () => { await deleteLoot(l.id); setLoots(await listLoots(selected.id)); })(); }}><Trash2 className="inline size-3.5" /></button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                {addingLoot ? (
                  <div className="grid gap-2 border-t border-white/10 p-3 sm:grid-cols-2 lg:grid-cols-5">
                    ...
                    <select className="field-select h-8 text-sm" value={lootKind} onChange={(e) => setLootKind(e.target.value as "market" | "npc")}><option value="market">Market</option><option value="npc">NPC</option></select>
                    <Input placeholder="Price" type="number" value={lootPrice} onChange={(e) => setLootPrice(e.target.value)} className="h-8 text-sm" />
                    <label className="btn-ghost h-8 cursor-pointer"><Upload className="size-3" /> Icon<input type="file" accept="image/*" className="hidden" onChange={(e) => setLootFile(e.target.files?.[0] ?? null)} /></label>
                    <button type="button" disabled={busy || !lootName.trim()} onClick={() => void onCreateLoot()} className="btn-primary h-8 sm:col-span-2 lg:col-span-1">Add</button>
                  </div>
                ) : null}
              </section>

              <section className="glass p-3">
                <p className="mb-2 text-[0.6rem] font-bold uppercase tracking-[0.18em] neon-text">Sessions — {selected.name}</p>
                        ...
                        {editingId === s.id ? (
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <div className="min-w-0">
                              ...
                              <p className="text-[0.65rem] text-muted-foreground">{new Date(s.created_at).toLocaleString()} · {s.minutes}m · {formatSilver(Number(s.total_value))} ⚙</p>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <span className="rounded-full bg-violet-400/15 px-2 py-0.5 text-xs font-semibold tabular-nums text-violet-300">{formatSilver(Number(s.silver_per_hour))}/h</span>
                              <button type="button" className="text-muted-foreground hover:text-cyan-300" onClick={() => startEdit(s)}><Pencil className="size-3.5" /></button>
                              <button type="button" className="text-muted-foreground hover:text-rose-400" onClick={() => { void (async () => { await deleteSession(s.id); setSessions(await listSessions(100)); })(); }}><Trash2 className="size-3.5" /></button>
                            </div>
                          </div>
                        )}
                      </li>
