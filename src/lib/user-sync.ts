...
/** In-memory cache so cloud-only tabs don't spin on every remount within the session. */
const cloudOnlyCache = new Map<string, unknown>();
...
export function useCloudOnlyStorage<T>(key: string, initial: T) {
  const [value, setValueState] = useState<T>(() =>
    cloudOnlyCache.has(key) ? (cloudOnlyCache.get(key) as T) : initial,
  );
  const [hydrated, setHydrated] = useState(() => cloudOnlyCache.has(key));
  ...
  const setValue = useCallback(
    (next: T | ((prev: T) => T)) => {
      setValueState((prev) => {
        const resolved = typeof next === "function" ? (next as (p: T) => T)(prev) : next;
        latest.current = resolved;
        cloudOnlyCache.set(key, resolved);
        if (timer.current) clearTimeout(timer.current);
        timer.current = setTimeout(() => {
